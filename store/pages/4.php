    ...
    <article style="background: url(<?php echo $url;?>medias/store/icons/background.svg)">
      <div class="ticket-container">
        ...
        <div class="response-container">
          <?php
          if( $error === false ) {
            echo '<div class="error">' . Language::string( 40, null, "store", null, $lang_code ) . '</div>';
          }
           ?>
          <div class="headline">
            <?php
            if( $ticket->values()["payment"] != 2) { //No payment required
              ...
              echo '<span' . Language::string( 41, null, "store", null, $lang_code ) . '></span>';
            } elseif ( $transaction["transaction_retrieve_status"] == false ) {
              echo '<img src="' . $url . 'medias/store/icons/error.svg" />';
              echo '<span>' . Language::string( 42, null, "store", null, $lang_code ) . '</span>';
            } elseif ( $transaction["status"] == "confirmed") {
              echo '<img src="' . $url . 'medias/store/icons/success.svg" />';
              echo '<span>' . Language::string( 43, null, "store", null, $lang_code ) . '</span>';
              ...
              echo '<span>' . Language::string( 44, null, "store", null, $lang_code ) . '</span>';
            } elseif ($transaction["pspId"] == 27) { //http://developers.payrexx.com/docs/miscellaneous
              echo '<img src="' . $url . 'medias/store/icons/waiting.svg" />';
              echo '<span>' . Language::string( 45, null, "store", null, $lang_code ) . '</span>';
            } else {
              echo '<img src="' . $url . 'medias/store/icons/error.svg" />';
              echo '<span>' . Language::string( 46, null, "store", null, $lang_code ) . '</span>';
            }
             ?>
          </div>
          ...
          <div class="details">
            <!-- Preis -->
            <div class="item">
              <span class="info"><?php echo Language::string( 52, null, "store", null, $lang_code ); ?></span>
            ...
            if(! empty( $ticket->values()["coupon"] )) {
              ...
              $coupon = Language::string( 53, null, "store", null, $lang_code );
            }
             ?>
            <div class="item">
              <span class="info"><?php echo Language::string( 54, null, "store", null, $lang_code ); ?></span>
              <span class="value"><?php echo $coupon; ?></span>
            </div>

            <?php
            if( $ticket->values()["payment"] != 2 ) { //Payment done
                $payment_state = Language::string( 55, null, "store", null, $lang_code );
                $payment_time = isset($ticket->values()["payment_time"]) ? date("d.m.Y H:i", strtotime( $ticket->values()["payment_time"] )) : '--.--.---- --:--';
            } elseif ( $transaction["transaction_retrieve_status"] == false ) {
              $payment_state = Language::string( 56, null, "store", null, $lang_code );
              $payment_time = '--.--.---- --:--';
            }else {
              switch($transaction["status"]) {
                ...
                break;
                case "confirmed":
                  $payment_state = Language::string( 57, null, "store", null, $lang_code );
                break;
                case "authorized":
                  $payment_state = Language::string( 58, null, "store", null, $lang_code );
                break;
                case "reserved":
                  $payment_state = Language::string( 59, null, "store", null, $lang_code );
                break;
                default:
                  $payment_state = Language::string( 60, null, "store", null, $lang_code );
                break;
              }
            ...
            <!-- Status -->
            <div class="item">
              <span class="info"><?php echo Language::string( 61, null, "store", null, $lang_code ); ?></span>
              <span class="value"><?php echo $payment_state; ?></span>
            </div>

            <!-- Zahldatum -->
            <div class="item">
              <span class="info"><?php echo Language::string( 62, null, "store", null, $lang_code ); ?></span>
              <span class="value"><?php echo $payment_time; ?></span>
            </div>

            <!-- Total -->
            <div class="item total">
              <span class="info"><?php echo Language::string( 63, null, "store", null, $lang_code ); ?></span>
              <span class="value"><?php echo number_format(($ticket->values()["amount"] / 100), 2) . ' ' . $group->values()["currency"]; ?></span>
            </div>
          </div>

          <div class="footer">
            <?php echo Language::string( 64, null, "store", null, $lang_code ); ?>
          </div>
        </div>
