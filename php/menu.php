    ...
    $nav .= '<nav>';
      ...
      while($mainMenuElement = $mainMenu->fetch()) {
        ...
        if( User::w_access_allowed($mainMenuElement["id"], $current_user) || User::r_access_allowed($mainMenuElement["id"], $current_user) ){
          $nav .= '<div class="page-container"><a onclick="openMenu(' . $mainMenuElement["id"] . ')" title="Menu #' . $mainMenuElement["id"] . ' [' . $mainMenuElement["name"] . ']">' . $mainMenuElement["name"] . '</a>';
          ...
          while($subMenuElement = $subMenu->fetch()) {
            if( User::w_access_allowed($subMenuElement["id"], $current_user) || User::r_access_allowed($subMenuElement["id"], $current_user) ) {
              ...
              }elseif(file_exists( dirname(__FILE__, 2) . '/plugins/' . $subMenuElement["plugin"] . '/' . $subMenuElement["image"] ) && is_file( dirname(__FILE__, 2) . '/plugins/' . $subMenuElement["plugin"] . '/' . $subMenuElement["image"] ) ) {
                $img_subpage= $url . 'plugins/' . $subMenuElement["plugin"] . '/' . $subMenuElement["image"];
              }else {
                $img_subpage= $url . 'medias/logo/favicon-color-512.png';
              }

              $nav .= '<div class="subpage-container ' . $class . ' mainpage'  . $mainMenuElement["id"] . '"><a href="' . $url . '?id=' . $mainMenuElement["id"] . '&sub=' . $subMenuElement["id"] . '" title="Submenu #' . $subMenuElement["id"] . ' [' . $subMenuElement["name"] . '] von Menu #' . $mainMenuElement["id"] . '"><img src="' . $img_subpage . '" />' . $subMenuElement["name"] . '</a></div>';
            }
          }
      ...
      //Profil menu
      $nav .= '<div class="page-container"><a href="' . $url . '?id=profil" title="Profil of you">Profil</a></div>';
